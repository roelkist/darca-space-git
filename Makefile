...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-space-git-venv
    export POETRY_HOME := /tmp/poetry-space-git-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-space-git-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/space-git-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-space-git-cache
endif
