...
class SpaceGitManager:
    """
    Provides Git operations scoped to darca-space-managed logical spaces.
    This manager ensures that all git operations occur within spaces allocated
    via darca-space-manager. No direct filesystem access is exposed to users.
    """
    ...
    def checkout_path_from_branch(
        self, space_name: str, paths: Union[str, List[str]], branch: str, dry_run: bool = False
    ) -> bool:
        if isinstance(paths, str):
            paths = [paths]
